...
(Monte-Carlo $\lambda\,$ Batch) Let $\lambda \in R^+$. Take a batch $\cal B$ of $B$ row vectors $\pi \sim \text{Unif}(-\lambda,\lambda)$ and denote it $\mathcal{B} = \{\pi_i\}_{i=1}^B$. Then, we have a $\lambda-\text{batch}$ denoted $\mathcal{B}_\lambda$. 

To simulate the eigenvectors, we will take the $Q-\text{evolution sequences}$ on each of our $\pi_i \in \mathcal{B}$. So, our simlated batch of evolution sequences is given by: 
$$\text{EvolBatch}(Q,\mathcal{B}) = \{\{\pi'_n\}_{n \in \N}\}_{i = 1}^N$$

Question: Which $\lambda$ is sufficient to obtain eigenvectors that yield all the eigenvalues $\lambda_i$ of $Q$?
