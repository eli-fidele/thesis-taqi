...
As the name suggests, these statistics are concerned with the distribution of the spacings between the eigenvalues.
...
In the future section where we define pairing schemes, we will introduce a new notation of eigenvalue pairs with one index when defining the consecutive pairing sch $\Pi_C$, and it takes the form $\pit_j$.
This notation is used to denote the consecutive eigenvalue pair for a given matrix.
The consecutive eigenvalue pairs are so special that we denote them with a unique notation for convenience. It also makes the discussion regarding order statistics more intrinsic.
...
So, whenever one sees the notation $\pit_j$, one should think of the $j^{th}$ largest eigenvalue and its smaller neighbour.
...
While we have defined dispersion metrics to be functions from $\Cc^2$, there is one special case where we can make an exception so that the domain of $\delta$ is not $\R^+$.
...
For this reason, we won't consider the arithmetic difference a formal dispersion metric, but we will honor it as a dispersion heuristic.
As such, we will denote this as the ``identity difference'' heuristic and call it $\delta_{\text{id}}$. So, we define $\delta_{\text{id}}: (z, z') \mapsto z' - z$
\end{remark}
...
\noindent *Note that the identity difference is a heuristic, and not a formal metric.
...
Before we may begin talking about pairing schema we define an auxiliary object, the spectral pairs of a matrix, which we denote $\spair$.
...
\begin{enumerate}
  ...
  \item The unique pair combinations schema are two complementary pair schema. By specifying $\textbf{either}$ $i > j$ or $i < j$,
    ...
    \item For completeness, we will also define the upper-pair scheme. $\Pi_<$ is the set of upper-pair unique combinations of ordered eigenvalues.
      However, it won't be used because we want dispersions to be positive-definite.
    $$\spair(P \mid \Pi_<) = \{\pi_{ij} = (\lambda_i,\lambda_j) \mid i < j\}_{i = 1}^{N-1}$$
    \end{enumerate}
...
It may cloud understanding initially but the payoff comes if you consider the implementation of the functions in \RMAT.
...
\noindent Another way we can define the pairing scheme is defining an auxiliary object: the eigenvalue (pair) matrix.

%****************************************************************************************%
\medskip
\minititle{An Alternative Representation: Eigenvalue Matrix}
%****************************************************************************************%
...
are redundant to include. We are only interested in comparing eigenvalues with other eigenvalues. This is because every the dispersion of eigenvalue with itself is zero,
...
In our notation, we are simulating the dispersion of $P \sim \Normal(0,1) \where P \in \R^{5 \times 5}$.
...
In our notation, we are simulating the dispersion of $\Ens \sim \H(\b = 4) \where P \in \R^{5 \times 5}$.
...
this works when $\d$ is a symmetric function. However, if we are concerned about spacings, then we must take into consideration a few facts.
\newline
\medskip
...
However, if we choose the appropriate pairing scheme, we can remove the absolute value to obtain a more meaningful upper bound. \newline
...
can be expressed as a linear combination of other elements. To avoid this issue of linear combinations, we simply take the dispersion with respect to the consecutive pairing scheme. In other words,
...
Formally speaking, we may define $\sim_\rankdiff$ as an equivalence relation. It satisfies all three properties of reflexivity, symmetry, and transitivity.
...
Recall that the ranking difference partitions the eigenvalue pair into separate classes based on how many ``ranks'' apart two eigenvalues are.
...
Suppose $P$ is an $N \times N$ symmetric matrix, and $\sigma(P)$ are its real, sign-ordered eigenvalues.
Then, the Wigner dispersion denoted $\disp_{W}(P)$ is given by the set of normalized consecutive eigenvalue spacings of $P$. That is,
$$ \disp_{W}(P) = \left\{ \frac{\d_n(\pi)}{\meanspacing} \mid \pi \in \spair(P \mid \Pi_C) \right\} $$
\end{definition}

The extension for ensembles is trivial; it inherits the same notation for matrices and the definition is extended similarly to how we
...
matrices are represented by the $\b$-ensemble with $\b = 2$! This will be discussed more in depth in \textbf{Chapter 4}.
...
This will be justified in \textbf{Chapter 4} in addition to being generalized for $\b \in \N$. In fact, this observation
...
At this point, the reader has acquired a vast toolkit and considerable knowledge to study random matrix theory in more depth.
