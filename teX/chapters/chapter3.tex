...
\begin{remark}[Spectrum Subset] If we denote $\mathbb{S} = \sigma(P)$ as the $\mathbb{ordered}$ spectrum of some matrix $P$, then for any valid pairing scheme $\Pi$, we will find that $\tilde{\sigma}(P \mid \Pi) \subseteq \mathbb{S}^2$ by definition. Note the emphasis on ordered - otherwise, the indices are meaningless.
\end{remark}
