...
In otherwords, we simply perform entry-wise sampling from the corresponding r.v. distribution. Note that this means by construction, $\D$ can only be e.h.d. if it has a corresponding probability distribution for random variables.
...
As a general rule, when it comes to implicit $\D$-distributions, we are concerned less about the distribution of the actual matrix entries and moreso about its holisitic properties.
...
% \end{remark}
%
% Speaking of dependence, our $\D$-distribtions sometimes depend on the dimension of the matrix $N$.
%
...
Let $P \sim \D$ be an $N \times N$ matrix over $\F$. Then, $P$ is represented as a vector of $K$ random variables $\vec{X} = (\seqo[K]{X})$ for $K \leq N^2$ over $\F$. In other words, some or all the entries of $P$ are random variables, and their distributions are precisely determined by the $\D$-distribution.
...
care about the $\b$-matrices. This is because their original characterization is not in this matrix model with a non-homogenous $\D$-distribution. Rather, it is
...
This is ultimately is what characterizes the ensemble. Speaking of which, this leads in quite nicely to our discussion of our first spectral statistic: the spectrum of a matrix, which is concerned all about the distribution of eigenvalues.
