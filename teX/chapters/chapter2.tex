...
$$ X_1,X_2,\dots,X_K \xra{\text{Make array}} (\vec{X} \rightsquigarrow P^*) \xra{\text{Compute} \det(P - \lambda I)} char_{P^*}(\lambda) \xra{\text{Solve for roots}} \sigma(P^*) $$
%=========================================================================================
...
Using the toolkit we have now accquired, we can now discuss an elegant, visual representation of the Perron-Frobenius theorem (See A.x). To put it shortly, the Perron-Frobenius theorem, applied to stochastic matrices is a result that guarantees the existence of a stationary distribution to an ergodic Markov Chain (see A.x). Sparing the details, we can use the conjunction of two facts, that if true, yield the result for the Perron-Frobenius Theorem for ergodic Markov chains through stochastic matrices.
\begin{enumerate}
