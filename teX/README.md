...
- Preamble: the preamble of any `LaTeX` document is very critical; it sources packages, sets up environments and macros, and so much more. So, having a well-factored preamble was necessary. So, the preamble was factored as a directory, in which there is the master preamble file `preamble.tex` which sources smaller files that comprise it. A breakdown is provided below.
