...
\newcommand{\ALGstochrow}{
	...
	\begin{enumerate}
		...
		\item Assign $r \la \vec{X}$, and then normalize the row by dividing each entry by the row sum; so assign $(r) \la (r) / {\sum_{j = 1}^N{r_j}}$.
...
\newcommand{\ALGstoch}{
	...
	\begin{enumerate}
		\item To generate a stochastic square matrix $P$ of size $N \times N$, fix $N \in \N$.
...
\newcommand{\ALGerdos}{
	...
	\begin{enumerate}
		...
		\item{For each row $r$ in $\{1,\dots,N\}$, generate $deg(r) \sim \Bin(N-1,p)$.}
		\item{Randomly choose $N-deg(r)$ vertices, then set the entries $r_{j}$ in the $j$ columns to 0 to sever them.}
...
\newcommand{\ALGexplicit}{
	\begin{algorithm}[Homogeneous Explicit $\D$-Matrix] \hfill
	\begin{enumerate}
