...
RM_ensemble <- function(mat_type, args, size){
  ...
  n_args <- length(args)
  if(n_args == 1){
    ensemble <- replicate(n = size, expr = fxn(args[[1]]), simplify = F)
  }
  if(n_args == 2){
    ensemble <- replicate(n = size, expr = fxn(args[[1]], args[[2]]), simplify = F)
  }
  if(n_args == 3){
    ensemble <- replicate(n = size, expr = fxn(args[[1]], args[[2]], args[[3]]), simplify = F)
  }
  if(n_args == 4){
    ensemble <- replicate(n = size, expr = fxn(args[[1]], args[[2]], args[[3]], args[[4]]), simplify = F)
  }
  # Replicate with appopriate number of arguments
  # Return the ensemble
  ensemble
}
