...
```{r, echo = F}
# Testing parameter accuracy for RM_erdos
erdos_test <- function(N, p, stoch, size){
  ens <- RME_erdos(N = N, p = p, stoch = stoch, size = size)
  v <- purrr::map_lgl(ens, .isStochastic)  
  return(sum(v) / size) # Get result
}
```
...
```{r}
# Test accuracy
erdos_test(N = N, p = 0.5, stoch = T, size = 1000)
erdos_test(N = N, p = 0.5, stoch = F, size = 1000)
```
