...
Let $\F$ be a field, and fix some $M \in \N$. Let $\B_\lambda \subset \F^M$ be a uniformly random batch of points in $\F^M$ in the $M$-hypercube of length $\lambda$. That is, 
...
Suppose we sample a random point from $\B_\lambda$ to emulate a random point $\vec{v} \in \F^M$. Additionally, let $Q \in \F^{M \times M}$ be a transition matrix over $\F$. Fixing a maximum power ('time') $T \in \N$, define the evolution sequence of $\vec{v}$ as follows:
...
Let $\varepsilon \in \R^+$, and suppose we have an evolution sequence $(a[\vec{v}])_n$. Then, $a_n$ $\varepsilon$-converges at $N$ if:
$$\exists N \in \N : \forall n \geq N \mid  a_N \sim_\epsilon a_n$$
