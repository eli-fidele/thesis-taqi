...
To motivate intuitive mathematical notation, we read $d(z,z')$ as the dispersion of $z$ with respect to (or in reference to) $z'$. Since we use the upper-pair scheme $\Pi_>$, the smaller eigenvalue is to the left and the larger to the right. This way, we take the dispersion of $\pi_{12}$ for example, we'd read $d(\pi_{21})$ as saying we're taking the dispersion of $\lambda_2$ with respect to $\lambda_1$. If this were the norm metric, we'd evaluate $\delta_{21} = |\lambda_1| - |\lambda_2|$.
