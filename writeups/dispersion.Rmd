...
The dispersion of a matrix $P \in \F^{N \times N}$ with respect to a dispersion metric $d: \Cc \times \Cc \to \F$, call it $\Delta_d(P)$, is defined as follows. Suppose $\sigma_P$ is the ordered spectrum of $P$ where $\sigma_P = \{\lambda_1, \dots, \lambda_N\}$. Then, let $\Pi_> = \{\pi_{ij} = (\lambda_i,\lambda_j) \in \sigma_P \times \sigma_P \mid i > j\}$ be the set of unique lower-pair (<) combinations of ordered eigenvalues. Then, the dispersion of $P$ with respect to $d$ is simply the set $\Delta_d(P)=\{d(\lambda_i ,\lambda_j) \mid \pi_{ij} \in \Pi_<\}$. In the case we choose a different pairing scheme $\Pi_0$, we simply denote the dispersion as $\Delta_d(P, \Pi_0)$.
\end{definition}

\begin{definition}[Ensemble Dispersion]
If we have an ensemble $\Ens$, then we can naturally extend the definition of $\Delta_d(\Ens)$. To take the dispersion of an ensemble, simply take the union of the dispersions of each of its matrices. In other words, if $\Ens = \{P_i \sim \mathcal{D} \mid \F\}$, then $\Delta_d(\Ens) = \bigcup_{i=1}^N \Delta_d(P_i)$.
\end{definition}

## A Caveat: Eigenvalue Ordering

In our definition of matrix dispersion, we are given a matrix $P$ which has an "ordered" spectrum $\sigma_P$. It is paramount to know what ordering scheme $\sigma_P$ is using, because otherwise, the indices are meaningless! So, consider the two following schema:
...
\begin{align*}
\sigma_P = \{\lambda_j : \lambda_1 \geq \lambda_2 \geq \dots \geq \lambda_N\}_{j = 1}^N
\end{align*}
